...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    function renderAnalytics() {
        ...
        const indCount = filteredMemberships.filter(m => m.type === 'Personal').length;
        ...
        const categories = ['Corporate', 'Academic', 'Alumni', 'Individual'];
    ...
    function updateCategoryOptions() {
        ...
        if (subTypeInput.value === 'Organisation') {
            ...
            subCategoryInput.innerHTML = '<option value="Alumni">Alumni</option><option value="Individual">Individual</option>';
        }
    }
    ...
    function updateSubCategoryOptions() {
        ...
        if (cat === 'Corporate' || cat === 'Alumni') {
            ...
            // Academic (Org) or Individual (Personal)
    ...
    subForm.addEventListener('submit', (e) => {
        ...
        if (type === 'Organisation') {
            ...
            // Personal doesn't have GSTIN or Authority by default in this schema
        }
...
We would like to inform you that your ${(item.category === 'Other' ? 'Individual' : (item.category || item.type))} membership will expire on ${toBoldChars(formatDate(item.expiryDate))}. We kindly request that you renew your membership by making a payment of ${toBoldChars(formatCurrency(item.amount))} per annum as soon as possible. You can choose to pay online or by cheque. After making the payment, please send us the transaction details so that we can verify with our Institute Account Section whether the payment has been successfully processed. Once we have confirmed the payment, we will proceed with renewing your membership and creating new cards.
